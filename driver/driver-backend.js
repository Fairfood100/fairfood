...
async function toggleOnline(){
  online=!online;
  await api("/api/driver/status",{method:"POST",body:{online}});
  document.getElementById("onlineBtn").textContent=online?"Offline":"Online";
  loadAll();
}

// تصليح رابط خرائط جوجل ليفتح بشكل رسمي وصحيح
function navUrl(o,mode="restaurant"){
  const dest=mode==="restaurant"?(o.restaurant_address||""):(o.delivery_address||"");
...
// دالة جديدة أضفتها لك لترسل موقع السائق الفعلي للسيرفر تلقائياً
...
login().then(async()=>{
  online=true;
  await api("/api/driver/status",{method:"POST",body:{online:true}});
  document.getElementById("onlineBtn").textContent="Offline";
  loadAll();
  setInterval(loadAll,10000);
  setInterval(syncLocation,30000); // تحديث الـ GPS كل 30 ثانية
});
